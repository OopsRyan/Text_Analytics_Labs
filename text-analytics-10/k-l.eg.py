...
def kldiv(_s, _t):
    ...
    print(min(_s.values()),min(_t.values()),ssum)
    ...
    for t, v in _s.items(): 
        pts = v / ssum
        ptt = epsilon
        if t in _t:  #if the same word in document t
            ptt = gamma * (_t[t] / tsum)  # the posibility of the word is the frequency possibility in document t
...
story1 = "john fell down harry fell as-well down by the stream the sun shone before it went down mary was fine"
story2 = "bill fell down jeff fell too down by the river the sun shone until it sunk down belinda was ill"
story3 = "it is Sunday today there are some clouds in the sky but the sun is shining Mr. Smith is with his family"

print("KL-divergence between story1 and story2:", kldiv(tokenize(story1), tokenize(story2)))
print("KL-divergence between story2 and story1:", kldiv(tokenize(story2), tokenize(story1)))
# print("KL-divergence between story1 and story3:", kldiv(tokenize(story1), tokenize(story3)))
# print("KL-divergence between story3 and story1:", kldiv(tokenize(story3), tokenize(story1)))
# print("KL-divergence between story2 and story3:", kldiv(tokenize(story2), tokenize(story3)))
# print("KL-divergence between story3 and story2:", kldiv(tokenize(story3), tokenize(story2)))
